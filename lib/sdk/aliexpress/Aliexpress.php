...
ownerMemberId 	   String 	否 	商品所有人搜索字段。必须是输入所需查询的商品所有人loginId，切当前用户是所有人或所有人上级用户。
	...
	*/
	...
	/**
	 * api.uploadImage：图片上传接口
	 */
	public function uploadImg($file, $fileName) {
		set_time_limit(0);
		$apiName = 'api.uploadImage';
		$this->doInit();
		//$x	=	file_get_contents("test_xx.jpg");
		//echo $x;
		//exit;
		$apiInfo = $this->server.'/'.$this->rootpath.'/'.$this->protocol.'/'.$this->version.'/'.$this->ns.'/'.$apiName.'/'.$this->appKey;
//.'?access_token='.$this->access_token.'&fileName='.$fileName
		$code_arr=	array (
			'access_token'		=> $this->access_token,
			'fileName'			=> "test_xx.jpg",
			'imageBytes'		=> "@".realpath("test_xx.jpg"),
		);
		//var_dump(realpath("xx.jpg"))."<br>";
		return $this->curl($apiInfo,$code_arr);
	}
